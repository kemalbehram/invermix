...
function act_deact_comp(id)
{
	$.ajax
    ({
        url: "/admin/act_deact/company/"+id,
        type: "get",
        data: '',
        success:function(result)
        {
        ...
        error: function (xhr) {
            $('#err').html('Error actualizando compañía');
            $('#err').show().animate({ width: "30%" }, "1000").delay(2000).animate({ width: "-50px" }, "1000").fadeOut(50);
        }
    });
...
function set_inputs(num)
{
    ...
    for(i = 1; i<=num; i++)
    {
        div_str = div_str+'<div class="col-sm-6 mt-3">\
                                ...
                                <input type="number" name="'+i+'" value="" class="form-control" placeholder="" required>\
                            </div>';
    }
