<?php
    ...
    {
        $v = Session::get('val');
        $actIny = App\inyects::where('user_id', $v)->orwhere('usn', 'like', '%'.$v.'%')->orwhere('capital', $v)->orwhere('status', $v)->orwhere('package', $v)->orwhere('created_at', 'like', '%'.$v.'%')->orderby('created_at', 'desc')->whereNull('deleted_at')->paginate(50);
        Session::forget('val');
    }
    else
    {
